...
mouseintr(void)
{
    ...
    while(inb(MSSTATP) & 0x01){
        ...
        static uint click_count[3] = {0, 0, 0}; // [Left, Right, Middle]
        ...
        static uint state = 0;  // 0=IDLE, 1=WAITING_SECOND, 2=CONFIRMED_SINGLE
        extern uint ticks;  // Use existing global ticks  
        ...
        // Timeout check - reuse last_click_ticks[0] instead of first_click_tick
